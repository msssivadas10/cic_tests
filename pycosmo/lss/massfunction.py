...
ftable = FunctionTable( 'LinearBias' )

@addfunction( 'press74', ftable, fclass = 'bias', mdef = [ 'fof' ] )
def mf_Press74(sigma: Any):
    r"""
...
@addfunction( 'sheth01', ftable, fclass = 'bias', mdef = [ 'fof' ] )
def mf_Sheth01(sigma: Any):
    r"""
...
@addfunction( 'tinker08', ftable, fclass = 'bias', z = True, mdef = [ 'so' ] )
def mf_Tinker08(sigma: Any, z : float, Delta: float):
    r""" 
...
@addfunction( 'jenkins01', ftable, fclass = 'bias', mdef = [ 'fof' ] )
def mf_Jenkins01(sigma: Any) -> Any:
    """
...
addfunction( 'reed03', ftable, fclass = 'bias', mdef = [ 'fof' ] )
def mf_Reed03(sigma: Any) -> Any:
    """
...
@addfunction( 'warren06', ftable, fclass = 'bias', mdef = [ 'fof' ] )
def mf_Warren06(sigma: Any) -> Any:
    """
    Fitting function be Warren et al. (2006).
    """
    A, a, b, c = 0.7234, 1.625, 0.2538, 1.1982
    sigma = np.asfarray( sigma )
    return A * ( sigma**-a + b ) * np.exp( -c / sigma**2 )

@addfunction( 'crocce10', ftable, fclass = 'bias', z = True, mdef = [ 'fof' ] )
def mf_Crocce10(sigma: Any, z: float) -> Any:
    """
    Fitting function be Crocce et al. (2010).
    """
    Az = 0.580*( z + 1 )**-0.130
    az = 1.370*( z + 1 )**-0.150
    bz = 0.300*( z + 1 )**-0.084
...
@addfunction( 'courtin10', ftable, fclass = 'bias', mdef = [ 'fof' ] )
def mf_Courtin10(sigma: Any) -> Any:
    """
    Fitting function be Courtin et al. (2010).
    """
    A  = 0.348
    a  = 0.695
    p  = 0.1
...
def massFunction(m: Any, cm: Cosmology, z: float = 0.0, model: str = 'tinker08', mdef: str = 'fof', out: str = 'dndlnm') -> Any:
    ...
    m = np.asfarray( m )
    r = cm.lagrangianR( m ) # lagrangian radius

    sigma = np.sqrt( cm.variance( r, z ) ) # sigma( m )
    ...
    # compute dn( m )/dln( m ) and dn( m )/dm:
    dndlnm = fsigma * ( cm.rho_m( z ) / m ) * ( -cm.dlnsdlnm( r, z ) )
    if out == 'dndlnm':
        return dndlnm
    elif out == 'dndm':
        return dndlnm / m

    raise ValueError("invalid value for out: '{}'".format( out ))
